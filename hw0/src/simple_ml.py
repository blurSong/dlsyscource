...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    softmax_loss = np.log(np.sum(np.exp(Z), axis=1)) - Z[np.arange(Z.shape[0]), y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(num_batches):
        ...
        softmax[np.arange(X_batch.shape[0]), y_batch] -= 1
        theta -= lr * X_batch.T.dot(softmax) / X_batch.shape[0]
    
    if X.shape[0] % batch != 0:
        X_batch = X[num_batches * batch:]
        y_batch = y[num_batches * batch:]
        Z = X_batch.dot(theta)
        Z -= np.max(Z, axis=1, keepdims=True)
        softmax = np.exp(Z) / np.sum(np.exp(Z), axis=1, keepdims=True)
        softmax[np.arange(X_batch.shape[0]), y_batch] -= 1
